...
class RouterNode():
    ...
    def __init__(self, ID, sim, costs):
        ...
        self.routingTable = [[self.sim.INFINITY]*self.sim.NUM_NODES] * self.sim.NUM_NODES
    ...
    def recvUpdate(self, pkt):
        ...
        for i in range(self.sim.NUM_NODES):
            if i != self.myID:
                self.myGUI.println("Compute min cost to go to " + str(i) + " from " + str(self.myID))
                possibleCost = self.neighbourCosts[i]
                print(" Cost to go directly to "+ str(i) + " is "+ str(possibleCost))
                for j in range(self.sim.NUM_NODES):
                    if (j != i and j != self.myID):
                        possibleCost1 = self.neighbourCosts[j] + self.routingTable[j][i]
                        print("possible cost by " + str(j) + " to go to " + str(i)  +" is " + str(possibleCost1))
                        if possibleCost1 < possibleCost:
                            possibleCost = possibleCost1
                            self.nextHop[i]=j
                self.minCosts[i] = possibleCost
                print("Finally my min cost to go to "+ str(i) +" is " +str(self.minCosts[i]))
    ...
    def printDistanceTable(self):
        ...
        self.myGUI.println(str(self.routingTable) + "\n")
        self.myGUI.println("My neighbour costs are : "+ str(self.neighbourCosts) + "\n")
    ...
    def updateLinkCost(self, dest, newcost):
        self.myGUI.println("updated link cost to " + str(dest) + "(" + str(newcost) + ")")

        for i in range(self.sim.NUM_NODES):
            if i == dest:
                self.neighbourCosts[i] = newcost
        self.myGUI.println("My news neighbours costs are :"+str(self.neighbourCosts))
        ...
        for i in range(self.sim.NUM_NODES):
            if i!=self.myID:
                ...
                for j in range(self.sim.NUM_NODES):
                    if (j!=i and j!=self.myID):
                        possibleCost1=self.neighbourCosts[j] +self.routingTable[j][i]
                        self.myGUI.println("possible cost for "+ str(j)+ " is "+ str(possibleCost1))
                        if possibleCost1 < possibleCost:
                           possibleCost=possibleCost1
                self.minCosts[i]=possibleCost
